...
export function AppProvider({ children }) {
  ...
  useEffect(() => {
    ...
    resetInactivity();
  }, [currentScreen, screenParams, session, language]);
  ...
  function t(key, lang = language) {
    return STRINGS[lang]?.[key] ?? STRINGS.es[key] ?? key;
  }

  function goTo(screen, params = {}, options = {}) {
  ...
  function startSession(langOverride = languageRef.current) {
  ...
  function setLastAction(label, item = null, category = null) {
  ...
  function buildHelpEmailPayload() {
  ...
  function openHelp() {
    setHelpOpen(true);
    Analytics.log('help_requested', {}, { session, language });
    Robot.callLibrarian();
    sendHelpEmail()
      .then(() => {
        Analytics.log('help_email_sent', buildHelpEmailPayload(), { session, language });
      })
      .catch((error) => {
        Analytics.log(
          'help_email_failed',
